...
export class LocalMemoryRuntime {
  ...
  async recallMessage(agent, messages, signal) {
    try {
      ...
      // 置顶核心记忆:每个会话第一次召回时整体注入一次
      if (this.cfg.recallPinned !== false && !state.pinnedInjected) {
        state.pinnedInjected = true;
        const pinned = this.store.listPinned(20);
        if (pinned.length > 0) {
          const lines = pinned.map((r) => `- [pinned] ${clipText(r.text, this.cfg.displayMaxChars)}`);
          const content = [
            `<local-memory source="pinned">`,
            "用户置顶的核心长期记忆(始终有效,优先遵守;若与用户当前指令冲突,以当前指令为准):",
            ...lines,
            "</local-memory>",
          ].join("\n");
          ...
          return createUserMessage({
            ...
            source: { kind: "plugin", plugin: PLUGIN_SOURCE, form: "pinned" },
          });
        }
      }
      ...
      // RRF 后:有向量信号的结果均可注入(vecScore 已在 search 里按 recallMinScore 过滤)
      const hits = (result.results || []).filter((r) => r.vecScore != null);
      if (hits.length === 0) return null;

      const lines = hits.map((r) => {
        const preview = clipText(r.text, this.cfg.displayMaxChars);
        const label = r.match === "rrf"
          ? `rrf ${r.score.toFixed(3)}|vec ${r.vecScore.toFixed(3)}`
          : `vec ${r.vecScore.toFixed(3)}`;
  ...
  async addMemory({ text, tags, supersedes }) {
    ...
    if (supersedes) {
      const oldIds = String(supersedes)
        .split(/[,，;；\s]+/)
        .map((s) => s.trim())
        .filter(Boolean);
      if (oldIds.length > 0) {
        const changed = this.store.supersede(oldIds, record.id);
        if (changed > 0) {
          this.logger?.info?.(
            `[dsh-local-vector-memory] superseded ${changed} old memory(ies) by ${record.id}`,
          );
        }
      }
    }
  ...
  async updateMemory({ id, text, tags, pinned }) {
    if (!this.store.ok) throw new Error(this.store.error ? `记忆库不可用: ${this.store.error.message}` : "记忆库不可用");
    ...
    const record = this.store.update(existing.id, {
      ...
      pinned:
        pinned !== undefined && pinned !== null
          ? pinned === true || pinned === 1 || pinned === "1" || pinned === "true"
          : undefined,
