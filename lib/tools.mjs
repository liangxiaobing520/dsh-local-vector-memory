    ...
    () =>
      ctx.tools.register(
        defineTool({
          name: "memory_add",
          description:
            "把一条值得长期记住的信息写入本地向量记忆库(用户偏好、约定、决定、环境事实等)。用户说\"记住...\"时主动调用。文本会先本地 embedding 成向量,再存入 SQLite;embedding 服务不可用时仍会保存,之后可 memory_reindex 补向量。可用 supersedes 参数(旧记忆 id 列表)把过时旧记忆标记为已被取代。",
          parameters: {
            text: { type: "string", required: true, description: "完整、独立、带必要上下文的中文记忆陈述。" },
            tags: { type: "string", description: "可选标签,逗号分隔,如 \"偏好,环境\"。" },
            supersedes: { type: "string", description: "可选,逗号分隔的旧记忆 id 列表;新记忆会把这些旧记忆标记为已被取代(取代链)。" },
          },
          output: TEXT_OUTPUT,
          isConcurrencySafe: () => true,
          execute: async (args) => {
            const result = await runtime.addMemory({ text: args?.text, tags: args?.tags, supersedes: args?.supersedes });
            if (result.duplicate) return `未重复写入:${result.warning}`;
            if (result.conflict) return `未写入:${result.warning}`;
      ...
      ctx.tools.register(
        defineTool({
          ...
          execute: async (args, exec) => {
            ...
            const vectorHits = hits.filter((r) => r.vecScore != null).length;
            const lines = hits.map((r, i) => {
              const time = String(r.createdAt || "").replace("T", " ").slice(0, 19);
              const tags = r.tags?.length > 0 ? ` [${r.tags.join(", ")}]` : "";
              const score = r.match === "rrf"
                ? `rrf ${r.score.toFixed(3)}|vec ${r.vecScore.toFixed(3)}`
                : `${r.score.toFixed(3)}|${r.match}`;
              return `${i + 1}. [${score}] ${r.text}${tags} (${r.id}, ${time})`;
            });
      ...
      ctx.tools.register(
        defineTool({
          ...
          parameters: {
            ...
            includeSuperseded: { type: "boolean", description: "默认 false;true 时列出已被取代的记忆(修正链)。" },
          },
          output: TEXT_OUTPUT,
          isConcurrencySafe: () => true,
          execute: async (args) => {
            ...
            const rows = runtime.store.list({
              ...
              includeDeleted: asBool(args?.includeDeleted),
              includeSuperseded: asBool(args?.includeSuperseded),
            });
            ...
            const lines = rows.map((r, i) => {
              ...
              const sup = r.supersededAt ? ` [已被取代→${r.supersededBy || "?"}]` : "";
              const sess = r.sessionId ? ` [会话:${r.sessionId.slice(-6)}]` : "";
              return `${i + 1}. ${r.id} ${time} [${r.source}/${vec}]${tags}${pin}${sup}${sess}${del}\n   ${clipText(r.text, 300)}`;
            });
      ...
      ctx.tools.register(
        defineTool({
          ...
          execute: async (args) => {
            ...
            const deleted = runtime.store.forget(id, { soft: !asBool(args?.purge) });
      ...
      ctx.tools.register(
        defineTool({
          ...
          parameters: {
            ...
            pinned: { type: "boolean", description: "可选 true/false;置顶核心记忆会在每个会话首次召回时整体注入。" },
          },
          output: TEXT_OUTPUT,
          isConcurrencySafe: () => true,
          execute: async (args) => {
            ...
            try {
              const result = await runtime.updateMemory({ id, text: args?.text, tags: args?.tags, pinned: args?.pinned });
        ...
        defineTool({
          ...
          execute: async () => {
            ...
            return [
              ...
              `总数 ${stats.total} | 已向量化 ${stats.vectorized} | 未向量化 ${stats.missingVectors} | 软删除 ${stats.deleted} | 置顶 ${stats.pinned} | 已取代 ${stats.superseded}`,
    ...
    () =>
      ctx.tools.register(
        defineTool({
          name: "memory_extract",
          description:
            "调用云端 deepseek-v4-pro 从一段文本/对话中提取长期记忆并写入记忆库。只适合较短的资料(建议不超过 2000 字)。用户给一段资料说\"把这些记下来\"时使用;一句话直接 memory_add。",
          parameters: {
          ...
          execute: async (args) => {
            ...
            if (result.extracted === 0) return "云端模型没有提取到值得长期保存的记忆。";
